...
export function GoogleAccountCard({ profile }) {
  ...
  return <section className="card-panel overflow-hidden">
    ...
    <div className="flex flex-col items-center gap-5 p-6 text-center sm:flex-row sm:text-left">
      ...
      <div className="min-w-0"><h3 className="truncate text-xl font-black">{profile.displayName || profile.name}</h3><p className="mt-2 flex items-center justify-center gap-2 break-all text-sm text-slate-500 sm:justify-start"><FiMail className="shrink-0"/>{profile.email}</p><p className="mt-2 text-sm font-black text-blue-600">ID: {profile.userId}</p><p className="mt-1 text-xs text-slate-400">{connected ? 'Google orqali tizimga kirilgan' : 'Email orqali tizimga kirilgan'}</p></div>
    </div>
  </section>
}
