import { Trophy as Award, BarChart3, BookOpenCheck, Clock as Clock3, Target } from '../components/AppIcons'
...
export function ResultsPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[[BarChart3,'O‘rtacha test natijasi',`${average}%`],[BookOpenCheck,'Yakunlangan darslar',lessonCount],[Award,'Sertifikat prognozi',forecast],[Target,'Jamlangan XP',points]].map(([Icon,label,value])=><div key={label} className="card-panel p-5"><Icon className="h-5 w-5 text-blue-600"/><p className="mt-5 text-sm text-slate-500 dark:text-slate-300">{label}</p><p className="mt-1 text-3xl font-black">{value}</p></div>)}</div>
