...
export function HomePage() {
  ...
  return <div className="space-y-8">
    <section className="relative overflow-hidden rounded-[32px] bg-slate-950 px-6 py-12 text-white sm:px-10 lg:px-14 lg:py-16"><div className="absolute -right-20 -top-20 h-80 w-80 rounded-full bg-emerald-500/25 blur-3xl"/><div className="absolute bottom-0 right-1/3 h-52 w-52 rounded-full bg-blue-500/20 blur-3xl"/><div className="relative max-w-3xl"><span className="pill bg-white/10 text-emerald-300">Milliy sertifikatga tayyorgarlik</span><h1 className="mt-6 text-4xl font-black leading-tight sm:text-5xl">Xush kelibsiz, {name}!</h1><p className="mt-4 text-xl font-semibold text-slate-100">Bugun qaysi fandan tayyorgarlik ko‘ramiz?</p><p className="mt-3 max-w-2xl leading-7 text-slate-300">6-sinf darsliklari asosidagi qisqa konspekt, progress test va yakuniy imtihonlar bilan bilimlaringizni tizimli mustahkamlang.</p><div className="mt-8 flex flex-wrap gap-3"><Link to="/subjects" className="btn-primary px-6">Fanni tanlash <FiArrowRight/></Link><Link to="/results" className="btn-secondary bg-white/10 text-white hover:bg-white/20">Natijalarim</Link></div><div className="mt-9 flex flex-wrap gap-7 text-sm text-slate-300">{['5 ta faol fan','Har 3 darsda test','Yakuniy Mock Exam'].map(item=><span key={item} className="flex items-center gap-2"><FiCheckCircle className="text-emerald-400"/>{item}</span>)}</div></div></section>
